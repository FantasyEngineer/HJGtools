...
public class NumberPickerView extends View {
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            } else if (attr == R.styleable.NumberPickerView_npv_TextGravity) {
                mTextGravity = a.getInteger(attr, 0);//文字怎么展示，输入0，1
            }
        }
        Log.d("NumberPickerView", "mTextGravity:" + mTextGravity);
        a.recycle();
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mTextGravity == 0) {//默认居中
            mViewCenterX = ((float) (mViewWidth + getPaddingLeft() - getPaddingRight())) / 2;
        } else if (mTextGravity == 1) {//靠左展示
            mViewCenterX = getPaddingLeft();
        } else {
            mViewCenterX = ((float) (mViewWidth + getPaddingLeft() - getPaddingRight())) / 2;
        }
        int defaultValue = 0;
