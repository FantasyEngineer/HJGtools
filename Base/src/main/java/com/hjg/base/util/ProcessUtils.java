...
public class ProcessUtils {
    ...
    public static String getForegroundProcessName() {
        ActivityManager manager = (ActivityManager) HJGUtils.getContext().getSystemService(Context.ACTIVITY_SERVICE);
        ...
        if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (list.size() > 0) {// 有"有权查看使用权限的应用"选项
                try {
                    ApplicationInfo info = packageManager.getApplicationInfo(HJGUtils.getContext().getPackageName(), 0);
                    AppOpsManager aom = (AppOpsManager) HJGUtils.getContext().getSystemService(Context.APP_OPS_SERVICE);
                    if (aom.checkOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS, info.uid, info.packageName) != AppOpsManager.MODE_ALLOWED) {
                        HJGUtils.getContext().startActivity(intent);
                    }
                    if (aom.checkOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS, info.uid, info.packageName) != AppOpsManager.MODE_ALLOWED) {
                        L.d("getForegroundApp", "没有打开\"有权查看使用权限的应用\"选项");
                        return null;
                    }
                    UsageStatsManager usageStatsManager = (UsageStatsManager) HJGUtils.getContext().getSystemService(Context.USAGE_STATS_SERVICE);
                ...
                L.d("getForegroundApp", "无\"有权查看使用权限的应用\"选项");
            }
        }
        return null;
    }
    ...
    public static Set<String> getAllBackgroundProcesses() {
        ActivityManager am = (ActivityManager) HJGUtils.getContext().getSystemService(Context.ACTIVITY_SERVICE);
    ...
    public static boolean killBackgroundProcesses(String packageName) {
        if (StrUtil.isEmpty(packageName)) return false;
        ActivityManager am = (ActivityManager) HJGUtils.getContext().getSystemService(Context.ACTIVITY_SERVICE);
        List<ActivityManager.RunningAppProcessInfo> infos = am.getRunningAppProcesses();
